...
hold off
savefig('ECG input signal - frequency domain zoom')
print('ECG input signal - frequency domain zoom','-dsvg','-r0')
...
% baseline drift filtriranje signala
y_baseline_drift_filtered = filter(b, a, val);

% iscrtavanje signala nakon baseline drift filtriranja
figure
...
% power line noise filtriranje signala
y_power_line_noise_filtered = filter(b1, a1, y_baseline_drift_filtered);

% iscrtavanje signala nakon power line noise filtriranja
figure
...
% iscrtavanje amplitudske karakteristike filtra
figure;
plot(w, 20*log10(H));
